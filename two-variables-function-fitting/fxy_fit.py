...
class TwoVarRealFuncTrainData(tud.Dataset):
    x_train = []
    y_train = []
    z_train = []
    ...
    def __init__(self, train_dataset_filename):
            with open(args.train_dataset_filename) as csv_file:
                ...
                for row in csv_reader:
                    ...
                    self.z_train.append(float(row[2]))
...
def build_model():
    layers = []
    layers.append(nn.Linear(2, args.hidden_layers_layout[0]))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='fxy_fit.py fits a two-variables real function dataset using a configurable multilayer perceptron')
    ...
    dataset = TwoVarRealFuncTrainData(args.train_dataset_filename)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_num, batch_data in enumerate(dataloader):
            print('=', end = '')
            x_train = torch.FloatTensor(batch_data[0].float()).to(device=args.device)
            y_train = torch.FloatTensor(batch_data[1].float()).to(device=args.device)
            z_train = torch.unsqueeze(torch.FloatTensor(batch_data[2].float()), dim=1).to(device=args.device)
            xy_train = torch.stack((x_train.T, y_train.T)).T;
            prediction = model(xy_train)
            loss = loss_func(prediction, z_train)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
